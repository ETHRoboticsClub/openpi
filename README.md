...
# Piper Towel Policy Fine-Tuning

This repository fine-tunes the **π₀.₅** policy on the **Piper Towel** dataset and provides workflows to reproduce training, convert datasets, and serve trained policies.
...
| Hugging Face CLI (`hf`) | Must be logged in with permissions to read `ETHRC/piper_towel_v0_with_rewards` |
| *(Optional)* Weights & Biases | Set `WANDB_MODE=offline` to disable online logging |
...
```
./scripts/piper_towel_workflow.sh     SOURCE_REPO_ID=ETHRC/piper_towel_v0_with_rewards     TARGET_REPO_ID=local/piper_towel_v0_converted     EXP_NAME=piper_towel_full
```
...
| Artifact | Location |
|---------|----------|
| Normalization statistics | `assets/pi05_piper_towel/local/piper_towel_v0_converted/` |
...
```
HF_REPO_ID=JessieLoki/pi05-piper-towel CHECKPOINT_STEP=999 ./scripts/run_piper_policy.sh
```

```
uv run scripts/serve_policy.py policy:checkpoint   --policy.config=pi05_piper_towel   --policy.dir=checkpoints/pi05_piper_towel/piper_towel_full/999   --port=8000
```
...
```
uv run examples/simple_client/run_client.py     --host 127.0.0.1 --port 8000     --config pi05_piper_towel     --from-lerobot ~/.cache/huggingface/lerobot/local/piper_towel_v0_converted     --episode 0 --timestep 0
```
